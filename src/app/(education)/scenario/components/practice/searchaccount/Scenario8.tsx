"use client";                                                                                                                             
  ...
  import { useAccountList } from "../../../../../searchaccount-scenario/components/hooks/useAccountList";
  import type { AccountCard } from "@/types";
  import Modal from "@/components/common/Modal";
  import Button from "@/components/common/Button";
  import AlertModalContent from "../../../../../searchaccount-scenario/components/components/AlertModalContent";
  import CategoryBlock from "../../../../../searchaccount-scenario/components/components/CategoryBlock";

  type Scenario8Props = {
    onSelectAccount?: (acc: AccountCard) => void;
  };
  ...
  export default function Scenario8({ onSelectAccount }: Scenario8Props) {
    ...
    const [activeFilter, setActiveFilter] = useState<"deposit" | "woori-group">("deposit");

    const openModal = (msg?: string) => {
      setAlertMessage(msg ?? "해당 계좌에서는 이체가 불가합니다.");
        ...
        <main className="flex-1 overflow-y-auto px-[20px] pb-[40px]">
          <section className="mt-[16px] flex items-center justify-between">
            <nav className="flex items-center gap-[12px] text-[15px] font-semibold">
              <button className="rounded-[16px] bg-[#2F6FD9] px-[14px] py-[6px] text-white">계좌</button>
              <button className="text-gray-400">카드</button>
              <button className="text-gray-400">예치</button>
            </nav>
            ...
            <div className="flex items-center gap-[8px]">
              ...
              <button
                type="button"
                aria-label="새로고침"
                className="flex h-[28px] w-[28px] items-center justify-center rounded-full border border-gray-200 text-[14px] text-gray-500" onClick={refetch}                                                                                                           
              >
                ↻
              </button>
            </div>
            ...
            {QUICK_FILTERS.map((filter) => {
              ...
              return (
                <button
                  key={filter.value}
                  type="button"
                  onClick={() => setActiveFilter(filter.value as "deposit" | "woori-group")}
                  className={`rounded-[16px] px-[16px] py-[8px] text-[13px] font-medium ${
                    isActive ? "bg-[#2F6FD9] text-white" : "bg-[#F3F4F6] text-gray-600"
                  }`}
                >
                  {filter.label}
                </button>
          ...
          <section className="mt-[24px] space-y-[24px]">
            {depositAccounts.length > 0 && (
              <CategoryBlock
                title={`예금 ${depositAccounts.length}`}
                accounts={depositAccounts}
                onTransfer={(acc: AccountCard) => onSelectAccount?.(acc)}
              />
            )}

            {savingsAccounts.length > 0 && (
              <CategoryBlock
                title={`적금 ${savingsAccounts.length}`}
                accounts={savingsAccounts}
                onTransfer={(acc: AccountCard) => openModal(acc.disabledMessage)}
              />
            )}
