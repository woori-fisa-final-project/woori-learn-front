...
export default function Scenario5({ onSuccess, onMaxFail, onClose, maxAttempts = MAX_ATTEMPTS, }: Scenario5Props) {
  ...
  const maxFailFiredRef = useRef(false);
  ...
  useEffect(() => {
    if (failureCount < maxAttempts) return;
    if (maxFailFiredRef.current) return;

    maxFailFiredRef.current = true;
    setLocked(true);

    if (timerRef.current) clearTimeout(timerRef.current);
    timerRef.current = setTimeout(() => {
      void onMaxFail(failureCount);
    }, 200);
