  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 sm:h-16 bg-white border-b border-gray-200 z-50 flex items-center justify-between px-4 sm:px-6 md:px-8">
      ...
      <nav className="hidden lg:flex items-center gap-4">
        <button
          className={`px-4 py-2 rounded transition-colors text-sm font-medium ${currentSection === 'users' ? 'bg-primary-500 text-white' : 'text-gray-700 hover:bg-primary-50'}`}
          onClick={() => onNavigate('users')}
        >
          회원 목록
        </button>
        <button
          className={`px-4 py-2 rounded transition-colors text-sm font-medium ${currentSection === 'exchange' ? 'bg-primary-500 text-white' : 'text-gray-700 hover:bg-primary-50'}`}
