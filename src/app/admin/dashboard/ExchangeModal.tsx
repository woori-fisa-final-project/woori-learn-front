...
const ExchangeModal: React.FC<ExchangeModalProps> = ({ open, onClose, onApprove }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 flex items-center justify-center backdrop-blur-[4px] bg-black/55">
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-xs flex flex-col items-center">
        ...
        <div className="flex gap-4 w-full">
          ...
          <button className="flex-1 py-2 rounded bg-primary-400 text-white hover:bg-primary-500" onClick={onApprove}>승인</button>
        </div>
      </div>
    </div>
  );
